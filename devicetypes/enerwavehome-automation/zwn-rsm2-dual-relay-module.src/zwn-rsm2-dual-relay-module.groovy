...
definition (name: "ZWN-RSM2 Dual Relay Module", namespace: "EnerwaveHome Automation", author: "Enerwave Home Automation")
{
capability "Switch"
capability "Refresh"
...
// tile definitions
tiles {
(1..2).each { n -> standardTile("switch$n", "switch$n", canChangeIcon: true) {
...
def onOffCmd(value, endpoint = null) {
    [encap(zwave.basicV1.basicSet(value: value), endpoint),"delay 500", encap(zwave.switchBinaryV1.switchBinaryGet(), endpoint),]
}
...
def refresh() {
    delayBetween([encap(zwave.basicV1.basicGet(), 1), // further gets are sent from the basic report handler
